...
RESULT_DIR = ROOT / "work" / "confidence_refined_recheck" / "results"
ALLOWED = {"high", "medium", "low", "exclude"}


def load_results() -> dict[int, dict[str, str]]:
    result_files = sorted(RESULT_DIR.glob("exclude_recheck_*.result.json"))
    if not result_files:
        raise SystemExit("No exclude_recheck result files found.")
...
def main() -> None:
    ...
    for idx, row in enumerate(rows):
        if row.get("confidence_refined") == "exclude":
            target_count += 1
            result = merged.get(idx)
            if result is None:
                raise SystemExit(f"Missing recheck result for row_index={idx} device={row.get('device')}")
    ...
    if target_count != len(merged):
        raise SystemExit(f"Expected {target_count} rechecked results, found {len(merged)}")
    ...
    print(json.dumps({
        ...
        "rechecked_rows": target_count,
        "result_files": len(list(RESULT_DIR.glob('exclude_recheck_*.result.json'))),
