...
@device(
    ...
    description="Guessed driver for a Cytiva AKTA pure class chromatography system used for affinity purification of phage-derived proteins.",
)
class CytivaAKTAPure:
    ...
    def equilibrate_column(self, method_name: str, buffer_name: str, column_id: str) -> dict:
        """Prepare the affinity column and baseline conditions before sample loading."""
    ...
    def load_sample(self, sample_id: str, volume_ml: float, target_column: str) -> dict:
        """Load harvested expression material onto the purification column."""
    ...
    def wash_column(self, target_column: str, wash_buffer: str, wash_volume_ml: float) -> dict:
        """Run the wash phase that removes weakly bound contaminants."""
    ...
    def elute_fraction(self, target_column: str, elution_buffer: str, fraction_volume_ml: float = 1.0) -> dict:
        """Elute affinity-purified product into collected fractions."""
    ...
    def export_purification_report(self, run_id: str) -> dict:
        """Export chromatogram and fraction metadata for downstream binding validation."""
